...
def index():
    ...
    replied = db(Avaliacoes.timestamp_reply!=None).select(orderby=~Avaliacoes.timestamp_reply).as_list()
...
&chtt=Distribuição+de+conceitos+de+avaliações''' % (evals_stats['A']['num'], evals_stats['B']['num'],\
    ...
    return dict(users=users[:10], logged=logged[:20], chart_url=chart_url, replied=replied[:4])
