...
#@auth.requires_membership('Admin')
#def remove_disc_relation()
#    prof_id = request.vars['prof_id']
#    disc_id = request.vars['disc_id']
#    if request.wsgi.environ['REQUEST_METHOD'] == 'GET':
#        session.jump_back = request.env.http_referer
#    prof_disc = db((db.profs_discs.professor_id==prof_id)&(db.profs_discs.disciplina_id==disc_id)).select()
#    if prof_disc:
#        db(db.profs_discs.id==prof_disc.id).delete()
#        db.commit()
#    session.flash = T('Relação excluída com sucesso')
#    redirect(session.jump_back)
